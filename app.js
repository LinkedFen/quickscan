...
function contactValid(contact) {
  ...
  return Boolean(name && emailValid(email) && consent);
}
...
function allAnsweredAndValid(contactState) {
  const contact =
    contactState ||
  ...
  return allQuestionsAnswered() && contactValid(contact);
}
...
function updateValidationUi() {
  ...
  if (tried && !emailValid(email)) {
    ...
    emailError.textContent =
      'Gebruik een geldig e-mailadres (bijv. naam@domein.ext).';
...
async function notifyBackendOnce(_results) {
...
async function downloadPdf(_results) {
  const node = document.getElementById('results-grid');
  if (!node) return;
  const name = document.getElementById('name').value.trim();
  const email = document.getElementById('email').value.trim();
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = QuickscanExports;
}
...
if (typeof document !== 'undefined' && !globalObj.__QUICKSCAN_NO_AUTO_INIT) {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document
      ...
      .addEventListener('change', onContactChange);
